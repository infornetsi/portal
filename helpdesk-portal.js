\
// helpdesk-portal.js — AyudaInfornet (Docker + DATA_DIR for Render)
...
// === Persistencia ===
const DATA_DIR = process.env.DATA_DIR || __dirname; // Render: /data
...
// === Estáticos (logo, css) del código (no en DATA_DIR) ===
app.use(express.static(path.join(__dirname, 'public')));

// === Adjuntos en DATA_DIR ===
...
// === Sesiones persistentes en DATA_DIR ===
app.use(session({
...
app.use((err, req, res, next) => {
  if (err.code === 'EBADCSRFTOKEN') {
    return res.status(403).send('Sesión caducada. Recarga la página e inténtalo de nuevo.');
  }
  next(err);
});
...
<html lang="es"><head>
  ...
  <style>body{font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:#f6f7fb}.card{border:none;border-radius:1rem;box-shadow:0 10px 25px rgba(0,0,0,.06)}.navbar{box-shadow:0 4px 10px rgba(0,0,0,.05)}.container-narrow{max-width:1100px}</style>
</head><body>
<nav class="navbar navbar-expand-lg bg-white"><div class="container">
  <a class="navbar-brand fw-semibold d-flex align-items-center gap-2" href="/">
    <img src="/logo.png" alt="AyudaInfornet" height="32">
    <span>Helpdesk</span>
  </a>
...
<script>window.__CSRF__='${csrfToken}'</script><script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
</body></html>`;
}
...
function csrfTokenFor(req){ try { return req.csrfToken(); } catch { return ''; } }
...
app.get('/login', (req,res) => {
  const c = `<div class=row justify-content-center><div class=col-md-7><div class="card p-4 p-md-5">
...
app.get('/client', requireAuth, async (req,res)=>{
  ...
  const list = tickets.map(t=>`<tr>
    <td><span class="badge text-bg-light">${t.uuid}</span></td>
    <td>${t.subject}<div class="text-muted small">${t.category} • SLA: ${t.sla_hours}h</div></td>
    <td><span class="badge text-bg-secondary">${t.priority}</span></td>
    <td><span class="badge text-bg-light">${t.status}</span></td>
  ...
  <div class=row g-4>
    <div class=col-md-5>
      <div class="card p-4">
        ...
        <form method=POST action=/tickets enctype=multipart/form-data>
          ...
          <div class=mb-3><label class=form-label>Categoría</label><input name=category class=form-control placeholder="General / Acceso / Infra / App" required></div>
          <div class=mb-3><label class=form-label>Prioridad</label><select class=form-select name=priority required><option>Alta</option><option selected>Media</option><option>Baja</option></select></div>
    ...
    <div class=col-md-7>
      <div class="card p-4">
        <h2 class=h5 mb-3>Mis tickets</h2>
        <div class=table-responsive>
          <table class="table align-middle">
            <thead><tr><th>ID</th><th>Ticket</th><th>Prioridad</th><th>Estado</th><th>Creado</th></tr></thead>
            <tbody>${list || ''}</tbody>
          </table>
        </div>
      </div>
...
app.post('/tickets', requireAuth, upload.array('files', 5), async (req,res)=>{
  const { subject, description, priority, category, sla_hours, group_id } = req.body;
  const id = uuidv4().slice(0,8).toUpperCase(); const now = new Date().toISOString();
  const result = await run(db, `INSERT INTO tickets (uuid,user_id,subject,description,category,sla_hours,priority,status,group_id,created_at,updated_at)
    VALUES (?,?,?,?,?,?,?,'Abierto',?,?,?)`, [id, req.session.user.id, subject, description, category || 'General', Number(sla_hours)||72, priority, group_id || null, now, now]);
  ...
  if (transporter) {
    try {
      await transporter.sendMail({
        from: process.env.MAIL_FROM || 'helpdesk@localhost',
        to: req.session.user.email,
        subject: `[Helpdesk] Ticket creado ${id}`,
        text: `Hemos recibido tu ticket ${id}:\n\nAsunto: ${subject}\nPrioridad: ${priority}\nCategoría: ${category}\nSLA: ${sla_hours}h`
      });
...
// Admin con filtros y gestión
app.get('/admin', requireAdmin, async (req,res)=>{
  const { q, status, priority, category, from, to, group_id, assignee } = req.query;
  ...
  const tickets = await all(db, `SELECT t.*, u.name as user_name, u.email as user_email, g.name as group_name, a.name as assignee_name
    FROM tickets t
  ...
  const rows = tickets.map(t=>`<tr>
    ...
    <td>
      ...
      <div class="text-muted small">${t.category} • SLA ${t.sla_hours}h</div>
      <div class="small mt-1">${t.description.substring(0,160)}${t.description.length>160?'…':''}</div>
      ${t.id ? `<div class="small mt-1">Adjuntos: <a href="/admin/tickets/${t.id}/attachments">ver</a></div>`:''}
    </td>
    ...
    <td>
      <form method=POST action=/tickets/${t.id}/status class="d-flex gap-2 align-items-center">
        ...
        <select name=status class="form-select form-select-sm">
          ${['Abierto','En progreso','En espera','Resuelto','Cerrado'].map(s=>`<option ${s===t.status?'selected':''}>${s}</option>`).join('')}
        </select>
        <select name=assignee_user_id class="form-select form-select-sm">
          <option value="">— Asignar —</option>
          ${agents.map(a=>`<option value=${a.id} ${t.assignee_user_id===a.id?'selected':''}>${a.name}</option>`).join('')}
        </select>
  ...
  const filterBar = `
    <form class="row g-2 mb-3" method=GET action=/admin>
      <div class=col-md-3><input class=form-control name=q placeholder="Buscar (texto/ID)" value="${req.query.q||''}"></div>
      <div class=col-md-2><select class=form-select name=status><option value="">Estado</option>${['Abierto','En progreso','En espera','Resuelto','Cerrado'].map(s=>`<option ${s===req.query.status?'selected':''}>${s}</option>`).join('')}</select></div>
      <div class=col-md-2><select class=form-select name=priority><option value="">Prioridad</option>${['Alta','Media','Baja'].map(p=>`<option ${p===req.query.priority?'selected':''}>${p}</option>`).join('')}</select></div>
      <div class=col-md-2><input class=form-control name=category placeholder="Categoría" value="${req.query.category||''}"></div>
      <div class=col-md-1><input class=form-control type=date name=from value="${req.query.from||''}"></div>
      <div class=col-md-1><input class=form-control type=date name=to value="${req.query.to||''}"></div>
      <div class=col-md-2><select class=form-select name=group_id><option value="">Grupo</option>${groups.map(g=>`<option value=${g.id} ${String(g.id)===String(req.query.group_id||'')?'selected':''}>${g.name}</option>`).join('')}</select></div>
      <div class=col-md-2><select class=form-select name=assignee><option value="">Responsable</option>${agents.map(a=>`<option value=${a.id} ${String(a.id)===String(req.query.assignee||'')?'selected':''}>${a.name}</option>`).join('')}</select></div>
      <div class=col-md-2><button class="btn btn-secondary w-100">Filtrar</button></div>
    </form>`;

  const c = `<div class=card p-4>
      <div class="d-flex align-items-center justify-content-between mb-2">
        <h1 class=h5 m-0>Panel de administración</h1>
      </div>
      ${filterBar}
      <div class=table-responsive>
        <table class="table align-middle">
          <thead><tr><th>ID</th><th>Ticket</th><th>Cliente</th><th>Prioridad</th><th>Grupo</th><th>Responsable</th><th>Gestión</th><th>Creado</th></tr></thead>
          <tbody>${rows || ''}</tbody>
        </table>
      </div>
    </div>`;
...
app.get('/admin/tickets/:id/attachments', requireAdmin, async (req,res)=>{
  ...
  const list = files.map(f=>`<li><a href="${f.path}" target=_blank rel=noopener>${f.filename}</a> <span class="text-muted">(${Math.round(f.size/1024)} KB)</span></li>`).join('') || '<li class=text-muted>No hay adjuntos</li>';
  ...
  res.send(layout('Adjuntos', req.csrfToken(), c, req));
});

// Actualizar estado/asignación (admin)
app.post('/tickets/:id/status', requireAdmin, async (req,res)=>{
  const { status, assignee_user_id, group_id } = req.body;
  if (status && !['Abierto','En progreso','En espera','Resuelto','Cerrado'].includes(status)) return res.status(400).send('Estado inválido');
  await run(db, 'UPDATE tickets SET status=COALESCE(?,status), assignee_user_id=COALESCE(?,assignee_user_id), group_id=COALESCE(?,group_id), updated_at=datetime("now") WHERE id=?', [status || null, assignee_user_id || null, group_id || null, req.params.id]);
